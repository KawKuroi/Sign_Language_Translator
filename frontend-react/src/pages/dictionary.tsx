...
function LetterCard({ letter, description }: { letter: string; description: string }) {
  return (
    <div className="bg-surface border border-border rounded-12 p-6 flex flex-col items-center text-center hover:border-ink3 transition-colors">
      <AslLetterSvg letter={letter} size={96} />
      <span className="font-serif italic text-48 text-ink mt-4 leading-none">{letter}</span>
      <p className="font-sans text-13 text-ink3 leading-[1.5] mt-3">{description}</p>
    </div>
  );
}
  ...
  return (
    <div className="w-full">
      <div className="mb-10">
        ...
        <h1 className="font-sans text-44 font-semibold text-ink mt-3 tracking-tighter2 leading-[1.05]">
          24 letras <span className="font-serif italic font-normal">estáticas</span>
        </h1>
        <p className="font-sans text-15 text-ink3 mt-3 max-w-xl">
